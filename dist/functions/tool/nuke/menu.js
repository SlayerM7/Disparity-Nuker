...
const spam_1 = __importDefault(require("./nuker/spam"));
function nukeMenu(client, rl) {
    ...
    rl.question(createString_1.createString("Enter option"), (option) => {
        ...
        rl.question(createString_1.createString("Enter server ID"), (guildId) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (option === "1") {
                let templateData = db_1.default.get(`template-${db_1.default.get("username")}`);
                deleteRole_1.default(client, rl, roles, guildId, false);
            ...
            if (option === "4") {
                rl.question(createString_1.createString("Enter channel names"), (channelName) => {
                    rl.question(createString_1.createString("Enter channel amount"), (channelAmount) => {
                        rl.question(createString_1.createString("Would you like to spam webhooks"), (spamWebs) => {
                            ...
                            create_channel_1.default(client, guildId, { amount: channelAmount, name: channelName }, rl, server, true, true);
                        });
                    });
                });
            }
            ...
            if (option === "7") {
                deleteRole_1.default(client, rl, roles, guildId, true);
            }
            if (option === "8") {
                spam_1.default(client, rl, channels, db_1.default.get(`template-${db_1.default.get("username")}`)
                    ? db_1.default.get(`template-${db_1.default.get("username")}`).webhookName
                    : "Disparity-Nuker", db_1.default.get(`template-${db_1.default.get("username")}`)
                    ? db_1.default.get(`template-${db_1.default.get("username")}`).webhookMessage
                    : "Disparity-Nuker", server);
            }
        }));
    });
}
